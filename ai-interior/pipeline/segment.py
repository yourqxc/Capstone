...
def box_coverage(rgba: Image.Image, box: tuple[int, int, int, int],
    ...
    **경고에만 쓴다.** 처음엔 이 판정으로 자동 크기를 껐으나 두 가지로 되돌렸다.
    (1) 크기를 바꿔도 갓만 남은 그림은 나아지지 않았다(10은 오히려 커졌다).
    (2) 사용자가 넉넉히 칠하면 멀쩡한 가구도 걸린다 — 침대를 넉넉히 칠하니 0.78.
    기준값은 딱 맞게 칠한 샘플로만 정한 것이라 실제 붓질에는 맞지 않는다. DEVLOG §25.
    """
