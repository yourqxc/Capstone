...
8. **정답으로 재 보니 크기가 3배 작았다** — 문을 자로, 깊이보다 촬영 관행이 나았다 (§29, §30)
9. **생성 모델을 붙여 보니** — 자연스러움과 가구 보존의 트레이드오프 (§26)
10. **범위와 한계** — 기획서 대비 무엇을 했고 무엇을 왜 뺐는가 (§21, §22, §23)
...
- 로컬 vs Gemini(배치 영역 CLIP): 로컬 +0.051 / 다듬기 +0.056 / Gemini +0.083 — 격차 4.6배 → **약 1.6배**. 원본 픽셀 유지 97% / 91% / 1% 미만
- 크기 공식을 ADE20K 문 180개로 검증: 깊이 지평선 ±25% 안 **5/62** → 사진 높이 41% 기준 **36/62** (방법을 고를 때 안 본 절반)
- 일반 실내 사진의 **31%**에서 바닥 평면을 못 찾는다 (바닥이 10% 이상 보이면 17%)
- 로컬 SD 다시 그리기: 통째로 하면 작은 가구 **0/4** 보존, 가구 주변만 키워서(강도 0.35) 하면 **4/4** 보존 · 한 장 28~43초 → 5~8초
...
**부정적 결과 두 가지.** (이후 §25에서 이 판정을 경고로만 바꿨다.)
...
| 한계 | 근거 | 왜 안 고쳤나 |
|---|---|---|
| **기울여 찍은 사진은 크기가 틀린다** (§29, §30 — 지평선을 사진 높이 41%로 가정) | 지평선을 사진 높이 74%(y=380)로 추정했는데 바닥-벽 경계가 76%(y=392). 작은 방인데 먼 벽이 아주 멀리 있는 것처럼 계산돼, 0.5m 테이블이 **25px**가 됐다 | 깊이 모델 출력은 상대 깊이라 알 수 없는 오프셋이 있고, 보이는 바닥이 좁으면 짧은 구간을 외삽하며 오차가 커진다. 고치려면 바닥-벽 선의 소실점 같은 다른 단서가 필요하다 |
...
| **가구 촬영 각도 ≠ 방의 원근** | 가구 사진은 한 방향에서 찍은 2D 이미지다. 방과 다른 각도로 틀어진 가구(예: 10시 방향 침대)는 보이지 않는 면을 만들어낼 수 없다 | 기하 합성의 근본 한계. SD로 다시 그려도 각도는 그대로였다(§26) — 깊이 조건이 붙여 넣은 합성본에서 오기 때문 |
| **바닥 과소·과대검출** | hold_10 재현율 0.327, hold_03 정밀도 0.664 (§18) | 홀드아웃으로 조정하면 홀드아웃이 무의미해진다. 새 튜닝에는 새 검증셋이 필요하다 |
| **CLIP이 사실감을 못 잰다** | 영역으로 잘라도 거대한 전등갓이 높은 점수(§23) | 같은 쌍 비교에만 쓰고, 사실감은 사람 채점으로 — 설문 준비 완료, 응답 수집 전(§31) |
...
| Stable Diffusion + ControlNet — 실사 합성·분위기 (7·8쪽) | **선택 기능 (§26, §27)** | 처음엔 제외(아래). 09-19 실험 후 기하 합성 뒤 "다듬기" 단계로 넣었다. 기본 꺼짐 |
...
## §25. 부분 누끼 판정을 경고로만 — 실제 붓질에서 오작동했다

`app.py`, `eval/run_eval.py`, `segment.box_coverage` · 09-19

**상황.** §20에서 "누끼 높이 ÷ 칠한 박스 높이 < 0.8이면 가구 일부만 잡힌 것"으로 보고 자동 크기를 껐다. 기준은 **딱 맞게 칠한 샘플 박스**로만 정했다(정상 1.00~1.09, 조명 0.58·0.47).

**측정.** 사용자 사진(`Sample/침대.jpg`)을 넉넉하게 칠하자 멀쩡한 침대가 **0.78**로 걸렸다. 실제 사용자는 가구를 딱 맞게 칠하지 않는다.

**결론.** 이 판정의 이득은 확인된 적이 없고(§20 — 크기를 바꿔도 갓만 남은 그림은 그대로), 손해는 확인됐다. 크기는 건드리지 않고 **경고만** 남겼다("넉넉히 칠해서라면 괜찮습니다"). 평가를 다시 돌리면 조명 07·10만 바뀐다(`local-005`).

> 기준을 정할 때 쓴 입력(딱 맞는 박스)과 실제 입력(사람의 붓질)이 달랐다. 샘플로 정한 기준은 샘플 밖에서 다시 확인해야 한다.

---

## §26. 로컬 Stable Diffusion으로 다시 그리기 — 하루 실험

`eval/experiments/sd_redraw.py` · 09-19

**상황.** 사용자 침대 사진을 넣으면 "누끼를 붙여 놓은 수준"이고 공중에 떠 보였다. 원인은 두 사진의 촬영 높이 차이다. 침대는 위에서 내려다보며 찍었고, 방은 낮은 눈높이에서 찍었다(지평선이 화면 80% 지점). 누끼를 붙이는 방식은 크기·위치·색·그림자는 맞춰도 **보는 각도는 바꿀 수 없다.**
기획서 8쪽의 ControlNet("방 구조를 유지하며 가구를 합성")이 원래 이 자리였다. §22에서 제외했던 것을 하루만 시험했다. 기하 파이프라인이 만든 합성본을 초기 이미지로, 가구 주변을 다시 그릴 영역으로, 합성본의 깊이맵을 ControlNet 조건으로 넣는다(SD 1.5 인페인팅 + ControlNet 깊이, fp16 약 2.9GB, 모두 로컬).
강도(0.35/0.6/0.85)와 샘플 4쌍(누끼 정상인 02·03·06·08)은 **결과를 보기 전에** 정했다.

**1차 — 침대, 이미지 통째로.** 한 장 24~39초, MPS 10.5GB.
- 0.35: 누끼 경계와 체커보드 조각이 사라진다. 여전히 떠 있다
- 0.6: 색과 경계가 녹아들고 침대 아래 그림자가 생긴다
- 0.85: 러그까지 그려 넣어 가장 바닥에 놓인 것처럼 보인다. 대신 헤드보드 색과 베개가 바뀐다
- **각도는 어느 강도에서도 그대로다.** 깊이 조건을 붙여 넣은 합성본에서 가져오므로 잘못된 시점이 조건으로 들어간다

![침대 다시 그리기](docs/evidence/23_SD_침대_다시그리기.png)

**2차 — 샘플 4쌍, 통째로 0.6/0.85.** 한 장 28~43초, MPS 12.1GB. **가구가 다른 물건이 됐다.** 06 유리문 책장은 베이지색 덩어리, 03 마호가니 의자는 빨간 좌판이 사라졌고, 02·08은 다른 디자인의 의자가 됐다. 영역 CLIP은 8건 모두 내려갔다.
원인은 해상도다. 침대는 화면 절반이었지만 샘플 가구는 화면 높이의 20~30%(100~150px)다. 이미지 전체를 512px 캔버스에 넣으면 가구에 남는 픽셀이 너무 적다.

**3차 — 가구 주변만 잘라 512px로 키워 다시 그리고 되돌려 붙이기.** 잘라내는 범위는 CLIP 평가와 같은 "배치 박스의 2배". 한 장 5~13초, MPS 7.2GB.

| | 가구 보존 (육안) | 방에 녹아드는 정도 | 한 장 |
|---|---|---|---|
| 통째로 0.6 | 0/4 (2개는 다른 물건, 2개는 다른 디자인) | 크다 | 28~35초 |
| 잘라서 0.6 | 2/4 (03 좌판 색, 06 책이 무늬로 바뀜) | 중간 | 9~13초 |
| **잘라서 0.35** | **4/4** | 작다 (경계·색이 부드러워지는 정도) | 5~8초 |

![작은 가구 보존](docs/evidence/24_SD_작은가구_보존.png)

**지표의 한계.** 가구 영역 픽셀 색차(ΔE)는 "잘라서 0.35"처럼 눈으로 거의 같은 경우에도 19~26이 나온다. 무늬가 조금만 밀려도 픽셀 단위로는 크게 달라지므로 보존 지표로 쓸 수 없다. 영역 CLIP도 쌍마다 오르내림이 섞였다. 이 실험의 판정은 육안이다.

**결론.**
1. **트레이드오프가 분명하다.** 생성 모델은 자연스러움(경계·그림자·접지)을 더하지만 가구의 정체성을 해친다. 정체성을 지키면(잘라서, 낮은 강도) 효과가 작다. "내가 살 이 가구가 내 방에 어울리는가"를 보여주는 서비스에서 가구가 바뀌면 안 된다.
2. **보는 각도는 이 방식으로 고쳐지지 않는다.** 깊이 조건이 붙여 넣은 합성본에서 오기 때문이다. 고치려면 방의 원근에 맞는 가구 형상(예: 바닥 평면 위 직육면체)을 조건으로 만들어야 하고, 그러면 가구가 더 크게 다시 그려져 정체성 문제가 커진다.
3. 기하 파이프라인의 역할이 분명해졌다. **무엇을 어디에 어떤 크기로 놓을지는 기하가 정확히 정하고, 생성 모델은 마감만 한다.** 순수 생성(§11의 Gemini)은 방 전체를 다시 그리고, 이 조합은 영역 밖을 원본 그대로 둔다.

재현: `python eval/experiments/sd_redraw.py --mode crop --strengths 0.35 0.6` (스크래치 실험과 CLIP 값 8개가 소수 넷째 자리까지 같음을 확인). `diffusers`가 필요하며 앱에는 아직 넣지 않았다.

---

## §27. 'AI 다듬기'를 앱의 선택 기능으로

`pipeline/refine.py`, `app.py` · 09-19

**결정.** §26의 결과로 사용자와 논의해 **선택 기능, 기본 꺼짐**으로 넣었다. 방식은 "가구 주변만 잘라 512px로 키워서", 기본 강도 0.35(샘플 4쌍 모두 가구가 알아볼 수 있게 남은 값). 강도 슬라이더는 0.2~0.8. 실험 스크립트(`eval/experiments/sd_redraw.py`)도 같은 `pipeline/refine.py`를 쓰게 바꿔 구현을 하나로 합쳤다. diffusers가 없으면 체크박스만 비활성이 되고 앱은 그대로 돈다.

**확인.**
- 끄면 결과가 이전과 **다른 픽셀 0개**
- 켜면 한 장 약 11~13초(M5, 첫 로드 포함)
- 같은 크기로 비교하면 경계와 조명은 자연스러워지지만, **강도 0.35에서도 의자 좌판 천이 선명한 빨강에서 무늬 있는 연분홍으로 바뀐다.** 모양은 유지된다. §26 표의 "보존 4/4"는 알아볼 수 있다는 뜻이지 그대로라는 뜻이 아니다

![앱 AI 다듬기](docs/evidence/25_앱_AI다듬기.png)

**함께 고친 것.** 가구 이름을 비워도 사진이 샘플이면 그 샘플의 높이로 자동 크기를 계산한다. 전에는 다듬기 단계만 샘플 정보를 쓰고 크기 계산은 이름으로만 찾아서, 이름을 비우면 크기가 박스 기준으로 바뀌었다(비교 사진을 만들다가 발견).

---

## §28. PR #1 병합과 재채점 — 전체 이미지 CLIP으로 낸 결론 두 개를 다시 잰다

`eval/angle_limit.py`, `eval/rescore_pilot.py`, `geometry.placement_check` · 09-19

**병합.** 09-09에 만든 PR(배치 검증, 각도 한계 실험, 유료 파일럿 결과)을 main에 병합했다. 분기 뒤 main이 크게 바뀌어 세 파일이 충돌했고 양쪽을 모두 살렸다. 앱은 `placement_check`로 **가구 밑면이 바닥 위에 있는가**를 새로 본다. §19에서 옮기기 전의 room_08 배치(소파 위)를 넣으면 "밑면이 바닥 영역과 0%만 겹칩니다"가 뜨고, 옮긴 배치에서는 뜨지 않는다.
함께 고친 것: "가구 높이가 화면의 18% 미만" 경고는 크기 자동 보정 이전에 만든 것이라, 자동으로 작게 계산된 경우에도 "더 크게 칠하라"고 했다. 이제 크기가 박스 기준일 때만 띄운다.

**각도 한계 실험 재채점.** 가구 사진 윗변 폭을 0.55~1.45배로 비틀어 촬영 높이가 다른 사진을 흉내 내는 실험이다. 처음엔 전체 이미지 CLIP에 기본 박스 누끼(01·03이 깨진 채)였다. 영역 CLIP, 가구 박스, 깊이 넘긴 합성으로 다시 쟀다.

| 윗변 폭 배율 | 0.55 | 0.70 | 0.85 | 1.00 | 1.15 | 1.30 | 1.45 |
|---|---|---|---|---|---|---|---|
| 처음 (전체 이미지) | +0.0053 | +0.0100 | +0.0083 | +0.0152 | +0.0184 | +0.0149 | +0.0147 |
| 재채점 (영역) | +0.0242 | +0.0327 | +0.0341 | **+0.0356** | +0.0386 | +0.0397 | +0.0318 |

"내려다보는 쪽(0.55)으로 비틀면 나빠진다"는 방향은 유지된다. 원본 대비 하락은 −65%에서 −32%로 줄었다. CLIP은 같은 방 안의 비교만 의미가 있으므로(§23) 방별로 세면 **5개 중 4개**에서 0.55가 원본보다 낮다(커피 테이블 room_05만 예외). 표본 5개라 경향으로만 쓴다.

**유료 파일럿 재채점 (API 다시 부르지 않음).** 저장된 Gemini 결과 9장을 영역 CLIP으로 다시 쟀다. 로컬은 파일럿 당시 결과가 지금 파이프라인과 달라서, 같은 배치 박스로 다시 돌렸다.

| 조건 | 영역 CLIP | 전체 CLIP (처음 방식) |
|---|---|---|
| 로컬 (파일럿 당시) | +0.032 | +0.012 |
| 로컬 (자동) | +0.034 | +0.015 |
| 로컬 (가구 박스) | +0.043 | +0.016 |
| 로컬 (가구 박스 + 다듬기 0.35) | +0.045 | +0.015 |
| Gemini (마커) | **+0.083** | +0.055 |
| Gemini (마스크) | +0.074 | +0.046 |
| Gemini (텍스트) | +0.069 | +0.040 |

세 쌍 모두 Gemini 세 조건이 로컬 네 조건보다 높다. 격차는 약 4.6배에서 약 2배로 줄었다. (§30 크기 기준 적용 후 다시 재면 로컬 가구 박스 +0.051, 다듬기 +0.056 — 격차 약 1.6배. §32) 로컬 안에서는 오늘 한 개선(가구 박스, 다듬기)이 순서대로 점수를 올렸다.

![파일럿 재채점](docs/evidence/26_파일럿_재채점.png)

**그림이 보여주는 것.** Gemini는 의자를 로컬보다 훨씬 크게 그렸다. CLIP이 영역 안에서 큰 의자를 보니 점수가 높다 — 격차의 상당 부분이 크기다. 그리고 이 그림에서 **로컬 쪽 크기가 작다**는 것이 드러났다. room_03에서 0.95m 의자가 뒤의 커피 테이블과 비슷한 높이다(§29).

---

## §29. 크기 자동 보정이 약 2배 작다 (→ §30에서 원인과 수정)

`geometry.auto_height_px` · 09-19

**측정.** 사진 속 물체의 화면 높이를 눈금으로 재고, 통상 실제 높이(0.45m)를 넣어 공식과 비교했다.

| 방 | 기준 물체 | 실측 | 공식 | 실측 / 공식 |
|---|---|---|---|---|
| room_03 | 커피 테이블 | 66px | 32px | 2.03 |
| room_01 | 플랫폼 침대 | 48px | 22px | 2.14 |
| room_01 | 낮은 협탁 | 70px | 39px | 1.82 |

실제 높이를 가정했으므로 ±20% 불확실하지만 2배는 그 안에 들어가지 않는다. 실측이 맞으려면 room_03의 지평선이 y=189(사진 높이의 37%)여야 하는데 추정은 y=293(57%)이다. §21의 room_09(지평선 74% vs 바닥 끝 76%)와 같은 양상 — **지평선이 먼 벽의 바닥선 근처로 너무 낮게 잡힌다.**

**틀린 가설.** 깊이맵을 최솟값 기준으로 0~1 정규화해서 "0 = 가장 먼 벽"이 되는 탓이라고 봤다. 정규화 전 출력의 최솟값이 0.13~0.24(범위의 1~2%)라 보정해도 지평선이 5~7px밖에 안 움직였다. 원인이 아니다.

§15의 "9/10 그럴듯함"은 육안이었고, "역깊이 비율과 일치"는 독립 검증이 아니었다(§15 정정). 절대 크기를 한 번도 정답과 대조하지 않았던 것이 이 오류를 3주 동안 숨겼다.

**다음 → §30.** 문 180개로 재 보니 오차는 가까운 가구의 2배보다 컸다(중앙값 3배). 지평선을 고정값으로 바꿨다.

---

## §30. 문을 자로 크기 공식을 검증 — 깊이로 찾은 지평선이 "고정값"보다 못했다

`eval/size_check.py`, `geometry.HORIZON_FRAC` · 09-19

**상황.** §29에서 크기 공식이 약 2배 작다는 것을 물체 세 개로 확인했다. 표본을 키우고 정답과 대조하려고 ADE20K 정답의 **문**을 자로 썼다. 문은 높이가 거의 표준(2.03m, ±5%)이다.

**방법 (결과를 보기 전에 정함).** 실내 씬(침실·거실·식당·홈오피스·호텔·기숙사) 가로 사진에서 샘플 20장을 빼고, 정답의 문 영역 중 위아래가 사진 끝에 닿지 않고, 바로 아래에 바닥이 30% 이상 있고(접지가 가려지지 않음), 세로/가로 1.5~5인 것. **문 180개, 방 163장.** 짝수 번째 90개(dev)로만 방법을 고르고 홀수 번째 90개(test)는 최종 확인에만 썼다. 문을 제대로 뽑는지는 12장을 눈으로 확인했다(천장까지 닿는 칸막이·통유리문 같은 이상치가 있어 중앙값으로 본다).

**지금 공식 (깊이 평면에서 외삽한 지평선).** test에서 크기가 계산된 문 62개 중 실측/공식 **중앙값 3.17**, ±25% 안 **5개(8%)**. 문이 가리키는 실제 지평선은 사진 높이의 33~45%에 모이는데, 깊이 추정은 51~62%로 아래에 치우쳤다.
...
|---|---|---|
| 지금 (깊이 지평선) | 6% | 33/90 |
| A. 배율 2.69배 곱하기 | 18% | 33 |
| B. 깊이 지평선을 사진 높이 18%만큼 올리기 | 36% | 31 |
| **D. 지평선을 사진 위에서 41%에 고정 (깊이 안 씀)** | **63%** | 0 |
| E. B와 D 평균 | 61% | 0 |

**test 확인 (앱과 같은 경로, 바닥 평면이 있어야 계산).** 크기가 계산된 62개 중 실측/공식 **중앙값 1.10**(사분위 0.94~1.35), ±25% 안 **36개(58%)** — 이전 5개. 나머지 28개는 바닥 평면을 못 찾아 두 방법 모두 계산하지 않았다.

![문 크기 검증](docs/evidence/28_문_크기검증.png)
![새 기준 합성](docs/evidence/27_크기보정_새기준.png)

**결론.**
1. **깊이 평면에서 외삽한 지평선은 크기 계산에 쓸 수 없었다.** 사람들이 방 사진을 눈높이에서 거의 수평으로 찍는다는 관행(지평선 ≈ 사진 높이의 41%)이 더 정확했다. 깊이맵은 여전히 바닥 영역, 접지선, 가림을 정한다 — 지평선만 바꿨다. 깊이 추정값은 `horizon_depth_y`로 남겨 기록한다.
2. 합성 결과에서 크기가 눈에 띄게 그럴듯해졌다. 0.95m 의자가 커피 테이블보다 높고(03), 1.8m 책장이 침대 머리판보다 높고(06), room_09의 0.5m 테이블이 점이 아니라 리클라이너 절반 높이다.

**한계.**
- 카메라를 크게 기울인 사진에는 맞지 않는다. 위에서 내려다본 room_01은 여전히 1.7배 작다(§29의 협탁·침대).
- **바닥 평면 실패가 일반 실내 사진의 31%다**(163장 중 50장). 샘플은 "바닥 10% 이상 보이는 사진"만 골라 이 문제가 안 보였다. 실패한 방의 정답 바닥 비율 중앙값 7.2% vs 성공 13.3%. 바닥 10% 이상이면 실패 17%, 15% 이상이면 10%. 앱은 "바닥이 더 보이는 사진을 써 주세요"로 멈춘다.
- 중앙값 1.10 — 약 10% 작게 나오는 경향이 남았다.
- §15의 "103 → 69 → 38 px"와 §17의 그림자 폭은 이전 지평선 기준의 수치다.

로컬 평가(`local-006`)는 가구가 커져 영역 CLIP이 자동 +0.0317 / 가구 박스 +0.0415가 됐다(`local-005`는 +0.0165 / +0.0256).

재현: `python eval/size_check.py` (약 5분, 요금 없음).

---

## §31. 사람 평가 설문 준비 — 로컬에서 띄우는 눈가림 설문

`survey.py`, `eval/survey/` · 09-19

**왜.** CLIP은 크기의 사실감도, 가구가 원래 제품 그대로인지도 재지 못한다(§23, §28). 로컬·다듬기·Gemini를 비교하는 표의 근거는 사람 채점이어야 한다.

**구성 (결과를 보기 전에 정함).**
- A. 방식 비교 — 유료 파일럿 3쌍 × {로컬, 로컬 + 다듬기 0.35, Gemini 마커} = 9문항
- B. 다듬기 효과 — 누끼가 정상인 5쌍(04·05·06·08·09) × {다듬기 없음, 있음} = 10문항
- 문항마다 1~5점 세 질문: 빨간 표시 자리에 놓였나 / 그 방에 실제로 있는 가구처럼 자연스러운가 / 가구 사진 속 제품과 같아 보이나
- 로컬 결과는 지금 파이프라인(§30 크기 기준 포함)으로 새로 만들었다. Gemini는 파일럿 이미지를 쓴다(다시 부르지 않음).

**눈가림.** 화면에 방식을 표시하지 않고, 참여자마다 순서를 섞는다. 처음엔 결과 이미지 파일 이름이 `A01g.jpg`라 이미지 주소로 Gemini인지 알 수 있었다 — 의미 없는 이름으로 바꿨다. 이름 등 개인정보는 받지 않고 임의의 참여자 번호만 남긴다.

**왜 claude.ai 페이지가 아니라 로컬 Gradio인가.** 응답을 모으는 공유 저장소는 같은 조직에 로그인한 사람만 쓸 수 있어 학과 친구들이 답할 수 없다. 로컬로 띄우면 계정 없이 링크만으로 답하고 응답은 이 컴퓨터에만 쌓인다(`--share`는 임시 공개 링크).

**검증.**
- 한 문항씩 답할 때마다 CSV에 바로 저장된다(중간에 나가도 남는다).
- 버그 하나를 테스트에서 잡았다: 답을 덜 채우고 "다음"을 누르면 오류를 던지게 했는데, gradio가 질문 칸까지 전부 "오류" 상태로 바꿔 **더 답할 수 없게 막았다.** 경고 알림만 띄우고 화면을 그대로 두게 고쳤다.
- 집계 스크립트는 효과를 심어 둔 가짜 응답 8명분으로 확인했다(Gemini 자연스러움 +1, 가구 보존 −1을 그대로 잡는다). 평균, 참여자 단위 부트스트랩 95% 구간, 같은 사람 안의 짝 비교를 낸다.

실행: `python survey.py` (또는 `--share`), 집계: `python eval/survey/summarize.py`. **응답은 아직 없다** — 5~10명이 필요하다.

---

## §32. 발표 자료 — 만들면서 다시 잰 숫자들

`../presentation/` · 09-19

10분 발표용 13장(`AI_셀프_인테리어_발표.pptx`)을 pptxgenjs로 만들고, 슬라이드마다 발표 대본을 메모로 넣었다. `build.js`로 다시 만들 수 있고 이미지는 `presentation/assets/`에 있다. 점검은 실제 PowerPoint로 PDF를 뽑아 한 장씩 봤다.

**만들면서 바로잡은 숫자.**
- **로컬 vs Gemini 재채점(§28)을 §30 크기 기준으로 다시 돌렸다.** 슬라이드의 그림은 새 기준인데 숫자는 옛 기준이었다. 로컬 가구 박스 +0.043 → **+0.051**, 다듬기 +0.045 → **+0.056**, Gemini 마커 +0.083 그대로. 격차는 약 2배 → **약 1.6배**.
- **원본 픽셀이 그대로인 비율을 3쌍에서 실측했다.** 처음엔 10쌍 평균(94%)을 그대로 쓰려 했는데, 다듬기는 가구 주변을 다시 그리므로 따로 재야 했다. 로컬 **96.6%**, 다듬기 **91.4%**, Gemini **0.2~0.6%**.
- **조명 정합 비교 그림을 다시 만들었다.** 증거 이미지 17번은 라벨이 한글 없는 글꼴로 그려져 네모로 깨져 있었고, 어느 칸이 어느 방식인지 확인할 수 없었다. 조명 정합을 고치기 전 코드를 git에서 꺼내 같은 침대로 세 방식을 다시 합성했다: 보정 없음 5.70 / 기존 6.68 / 새 방식 5.02 (§14 기록 5.79 / 6.73 / 5.09와 일치).

**PowerPoint 한글 줄바꿈.** 처음 만든 판은 "파이프라\n인"처럼 한글 단어 중간에서 줄이 바뀌었다. 문단에 `eaLnBrk="0"`을 넣어도 그대로였고, 원인은 pptxgenjs가 모든 글자에 언어를 영어(`en-US`)로 붙이는 것이었다. 글상자 언어를 `ko-KR`로 지정하자 단어 단위로 줄바꿈됐다.

**비어 있는 칸.** 11번 슬라이드(사람 평가)는 설계만 있고 결과는 "응답 수집 중"이다. 응답이 모이면 채운다.
