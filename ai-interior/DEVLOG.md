...
**결론.** 로컬 품질의 근거로 CLIP delta를 쓰면 안 된다. 누끼 정상 여부는 육안 판정이고, 사람 채점(K9)이 필요하다.
...
| **가림 처리 없음** | room_04에서 의자가 침대 발치 기둥 위에 그려진다 | 깊이맵은 이미 있다. 다음 작업 |
...
| **CLIP이 로컬 품질을 구별 못 함** | §19 | 사람 채점으로 대체해야 한다(미실시) |
