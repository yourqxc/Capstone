...
- 결과 파일 출처 판별: 가구 없는 영역 평균 절대차 1.02(합성물) vs 13.95(다른 사진)
...
→ 구멍은 **작은 것만** 메우도록 수정(마스크 면적의 0.5% 미만). 진짜 빈 공간은 크고 잡티는 작다.
...
- **조명·색조 불일치** — 컷아웃이 원래 촬영 조명을 그대로 유지해 방과 톤이 다르다. 합성 티가 나는 주된 원인.
...
## §11. 실사 방 사진에서 드러난 문제 — 그림자가 보이지 않았다

사용자가 직접 준비한 방/침대 사진으로 두 방식을 돌린 결과를 검토했다.

### 먼저 확인한 것: 결과 파일의 출처

`Output/local.jpg`가 로컬 파이프라인 결과로 보관돼 있었으나, **우리 출력이 아니었다.**
침대가 없는 우측 영역만 원본 방 사진과 비교했다.
...
가구가 없는 영역까지 다르면 같은 원본에서 나온 합성이 아니다. 해상도도 1234x864로
원본(740x518)과 다르고, 침대 위에 블라인드 그림자가 물리적으로 맞게 떨어져 있다 —
합성으로는 나올 수 없는 조명이다. 같은 목업 세트의 "침대가 놓인 원본 사진"이었다.

`gemini.jpg`는 차이 1.02로 실제 API 출력이 맞다.

> **교훈.** 결과 이미지를 눈으로만 보면 출처를 착각할 수 있다.
> 가구가 없는 영역을 원본과 수치로 비교하면 합성물인지 아닌지 바로 갈린다.
> 실험에서 결과 파일을 다룰 때 이 확인을 넣을 것.

### 진짜 문제 — 접지 그림자가 가구 뒤에 가려 있었다

우리 파이프라인으로 같은 조합을 재현하니 침대가 바닥에서 떠 보였다.
`contact_shadow`가 실루엣을 접지선 쪽으로 **누르기만** 해서, 만들어진 그림자가
가구 자신의 실루엣 안에 거의 전부 가려졌다. §9에서 "최대 농도 0.45, 면적 3079px"로
수치는 확인했지만 **가구 밖에서 보이는 면적을 재지 않았던 것**이 놓친 지점이다.

수정: 누른 뒤 **아래로 더 밀어내고**(drop) 가로로 살짝 넓히고(spread),
방의 밝기 분포로 광원 방향을 추정해 반대쪽으로 이동시킨다.
...
컷아웃이 원래 촬영 조명을 그대로 들고 오는 문제(§9의 남은 한계)를 부분적으로 해결했다.
가구의 LAB 평균을 방의 평균 쪽으로 **35%만** 이동시킨다. 밝기(L)는 그 절반만 옮긴다 —
전부 맞추면 가구의 형태감이 뭉개진다.

![그림자·색 조화 수정](docs/evidence/10_그림자_색조화_수정.png)
*좌: 수정 전 / 우: 수정 후. 아래는 접지부 확대*

### Gemini API 쪽 실패

`gemini.jpg`에서 침대가 화면 왼쪽 아래 구석에 폭 38% 크기로 잘려 들어갔다.
마커로 지정한 위치와 무관하게 배치된 것으로 보인다. 재현하려면 유료 호출이 필요해
아직 원인을 특정하지 못했다. **3주차 실험에서 다룰 항목이다.**
